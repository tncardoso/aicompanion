...
pub struct CallGraph {
    /// Adjacency list as a vec of (caller, callees) pairs (HashMap not serializable with struct keys).
    pub edges: Vec<(FnId, Vec<FnId>)>,
...
pub fn build(all_files: &[(String, String)], changed_paths: &HashSet<String>) -> CallGraph {
    ...
    let edges_vec: Vec<(FnId, Vec<FnId>)> = edges.into_iter().collect();
